...
def upsert_df(df: pd.DataFrame, table_name: str, engine: sqlalchemy.engine.Engine, **kwargs) -> bool:
    ...
    if not engine.execute(
        ...
        df.to_sql(table_name, engine, index=True, **kwargs)
        return True

    # If it already exists...
    temp_table_name = f"temp_{uuid.uuid4().hex[:6]}"
    df.to_sql(temp_table_name, engine, index=True, **kwargs)
    ...
    try:
        ...
        if not 'unique_constraint_for_upsert" already exists' in e.args[0]:
            # Drop temporary table if there is any error
